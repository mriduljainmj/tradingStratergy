...
const many=Array.from({length:50000},(_,i)=>({time:open+i*60,open:100,high:110,low:90,close:100}));
const last=many.at(-1);
const changed=merge(many,[{...last,close:105,last_tick:last.time+20}],'minute');
assert.equal(changed.length,many.length);
assert.equal(changed[0],many[0]);
assert.equal(changed.at(-2),many.at(-2));
